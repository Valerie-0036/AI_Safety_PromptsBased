...
import pathlib
import pandas as pd
from prompt_manipulation_tools import prompt_encoder, prompt_decoder
...
async def generate_google_genai_response(client: genai.Client, prompt, system_prompt):
...
async def send_prompt_to_model(prompt: str, client: genai.Client, system_prompt: str):
    tasks = [generate_google_genai_response(client, prompt, system_prompt) for _ in range(1)]
...
async def test_model(client, system_prompt, csv_path=None, should_print_prompts=True):
    ...
    Args:
        client: GenAI client. If None, a new client will be created.
    ...
    if csv_path is None:
        csv_path = os.path.join(script_dir, "example_prompts.csv")
    ...
    async def test_prompts():
        ...
        for idx, row in random_sample.iterrows():
            ...
            if should_print_prompts:
                print(f"Prompt text:\n {prompt_text}")
            result = await send_prompt_to_model(prompt_text, client, system_prompt=system_prompt)
            ...
            print("  With encoding:")
            encoded_prompt = prompt_encoder(prompt_text,encoding)  # Random encoding
            encoded_success = await send_prompt_to_model(encoded_prompt, client,system_prompt=system_prompt)
            print(encoded_success)
            
            # print("Decode:")
            # encoded_success = await send_prompt_to_model(encoded_success, client,system_prompt=system_prompt)
            # print(encoded_success)   
            # if encoded_success:
